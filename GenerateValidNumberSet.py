...
for i in range(9999):
    ...
    pad_array = np.pad(numpy_original, (4 - len(numpy_original), 0), 'constant', constant_values=(0, 0)) # Fill out the array with zeros if the number of digits is less than 4
    if len(np.unique(pad_array)) == 4: #Check if the array has four unique numbers
